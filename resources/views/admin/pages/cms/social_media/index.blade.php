...
@section('title','Social Media | Jiu Jitsu')
                            ...
                            @if($icons->count() > 0)
                                @foreach($icons as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if(!empty($item->image))
                                                <img src="{{ asset('storage/'.$item->image) }}" class="avatar-sm" alt="image">
                                            @else
                                                No Image Uploaded
                                            @endif
                                        </td>
                                        <td>
                                            <div class="d-flex gap-2">
                                                <button class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#editPrivacyPolicyModal"
                                                        onclick="editRecord('{{$item->id}}','{{$item->link}}','{{ asset('storage/'.$item->image) }}')"><i class="bx bx-edit"></i></button>
...
@section('custom-script')
    <script>
        ...
        $('#editPrivacyPolicy').on('submit', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('edit_social_media_request') }}",
                dataType: 'json',
                contentType: false,
                processData: false,
                cache: false,
        ...
        $('#deletePrivacyPolicy').on('submit', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('delete_social_media_request') }}",
                dataType: 'json',
                contentType: false,
                processData: false,
                cache: false,
