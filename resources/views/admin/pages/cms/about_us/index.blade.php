...
@section('title','About Us | Jiu Jitsu')
                            ...
                            @if($about_us->count() > 0)
                                @foreach($about_us as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                ...
                                                @foreach(json_decode($item->icon) as $icon)
                                                    <a href="{{ !empty($icon->icon_link) ? $icon->icon_link : 'javascript:void(0)' }}"><img class="display_icon" src="{{asset('storage/'.$icon->icon_image)}}" alt="icon_image"/></a>
                                                @endforeach
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                <button class="btn btn-sm btn-outline-success getAboutUs" data-id="{{ $item->id }}"><i class="bx bx-edit"></i></button>
...
@section('custom-script')
    <script>
        $('#addAboutUs').on('submit', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('add_about_us_request') }}",
                dataType: 'json',
                contentType: false,
                processData: false,
                cache: false,
                    ...
                    if (res.success === true) {
                        ...
                        if (data.icon) {
                            ...
                            decodedIconData.forEach(function (item,index) {
                                let icon_image_path = '{{ asset('image') }}'.replace('image', 'storage/' + item[index].icon_image);
        ...
        $('#editAboutUs').on('submit', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('edit_about_us_request') }}",
                dataType: 'json',
                contentType: false,
                processData: false,
                cache: false,
        ...
        $('#deletePrivacyPolicy').on('submit', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('delete_about_us_request') }}",
                dataType: 'json',
                contentType: false,
                processData: false,
                cache: false,
        ...
        function change_status(id, value) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('update_about_us_status') }}",
                dataType: 'json',
                data: {
                    id: id,
                    value: value,
