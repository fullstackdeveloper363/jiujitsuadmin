                            ...
                            @if($users->count() > 0)
                                @foreach($users as $item)
                                    <tr>
                                        ...
                                        <td>{{ !empty($item->name) ? $item->name : 'No Data' }}</td>
                                        <td>{{ !empty($item->email) ? $item->email : 'No Data' }}</td>
                                        <td>{{ !empty($item->gender) ? $item->gender : 'No Data' }}</td>
                                        <td>{{ !empty($item->training_affiliation) ? $item->training_affiliation : 'No Data' }}</td>
                                        <td>
                                            <div class="form-check form-switch">
                                                ...
                                                <label class="form-check-label"
                                                       for="flexSwitchCheckDefault">{{ $item->status == 1 ? 'Active' : 'In Active' }}</label>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex gap-2">
                                                <a href="{{ route('view-user-details',$item->id)}}" class="btn btn-sm btn-outline-warning" ><i class="bx bxs-eyedropper"></i></a>
                                                <button class="btn btn-sm btn-outline-success getUserDetail" data-bs-toggle="modal" data-bs-target="#editModal"
                                                        data-id="{{ $item->id }}"><i class="bx bx-edit"></i></button>
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.success === true) {
                        ...
                        {
                            let image_path = "{{ asset('images') }}".replace('images',res.data.profile_image)
